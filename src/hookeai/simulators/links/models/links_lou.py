...
#
# =============================================================================
class LinksLou(LinksConstitutiveModel):
    """Lou-Zhang-Yoon constitutive model.
    
    Lou-Zhang-Yoon model with general differentiable yield function and
    isotropic hardening.
    ...
    def _get_required_material_descriptors():
        ...
        material_descriptors = ('name', 'density', 'young', 'poisson',
                                'yield_a_init', 'a_hard_slope',
                                'yield_b_init', 'b_hard_slope',
                                'yield_c_init', 'c_hard_slope',
                                'yield_d_init', 'd_hard_slope',
    ...
    def _format_material_descriptors(self, mat_phase, descriptors):
        ...
        yield_a_init = descriptors['yield_a_init']
        a_hard_slope = descriptors['a_hard_slope']
        yield_b_init = descriptors['yield_b_init']
        b_hard_slope = descriptors['b_hard_slope']
        yield_c_init = descriptors['yield_c_init']
        c_hard_slope = descriptors['c_hard_slope']
        yield_d_init = descriptors['yield_d_init']
        d_hard_slope = descriptors['d_hard_slope']
        ...
        descriptors_input_lines = \
            ...
            [spacing + '{:<16.8e}'.format(yield_a_init) +
             '{:<16.8e}'.format(a_hard_slope) + '\n'] + \
            [spacing + '{:<16.8e}'.format(yield_b_init) +
             '{:<16.8e}'.format(b_hard_slope) + '\n'] + \
            [spacing + '{:<16.8e}'.format(yield_c_init) +
             '{:<16.8e}'.format(c_hard_slope) + '\n'] + \
            [spacing + '{:<16.8e}'.format(yield_d_init) +
             '{:<16.8e}'.format(d_hard_slope) + '\n']
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
