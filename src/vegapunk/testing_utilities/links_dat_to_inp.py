...
if not os.path.isfile(links_input_file_path):
    raise RuntimeError('Links simulation input data file has not been found.')
# Get simulation name
simulation_name = os.path.splitext(os.path.basename(links_input_file_path))[0]
...
for line in links_input_file:
    if bool(re.search(r'ELEMENT_TYPES\b', line, re.IGNORECASE)):        
        ...
        if n_elem_types > 1:
            raise RuntimeError('This script can only process meshes with a '
                               'single element type.')
...
# Initialize search flags
is_keyword_found = False
# Search for NODES_WITH_PRESCRIBED_DISPLACEMENTS keyword and collect Dirichlet
# boundary conditions
for line in links_input_file:
