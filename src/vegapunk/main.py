...
class FiniteElementPatch:
    ...
    def __init__(self, n_dim, patch_dims, elem_type, n_elems_per_dim,
                 ...
                 corners_coords_def, n_edges, edges_per_dim, edges_mapping,
        ...
        corners_mapping : dict
            For each corner label (key, str[int]), store the corner internal
            index. Corners are labeled from 1 to number of corners.
        corners_coords_ref : numpy.ndarray(2d)
        ...
        edges_per_dim : dict
            ...
            indexes of the two corresponding corners (sorted in asceding order
            of coordinate).
        edges_mapping : dict
        ...
        self._corners_mapping = copy.deepcopy(corners_mapping)
        self._corners_bc = copy.deepcopy(corners_bc)
        self._corners_coords_ref = copy.deepcopy(corners_coords_ref)
        self._corners_coords_def = copy.deepcopy(corners_coords_def)
        self._n_edges = n_edges
        self._edges_per_dim = copy.deepcopy(edges_per_dim)
        self._edges_mapping = copy.deepcopy(edges_mapping)
        self._edges_coords_ref = copy.deepcopy(edges_coords_ref)
    ...
    def plot_deformed_patch(self):
        ...
        fig, ax = plt.subplots()
        if self._n_dim == 2:
            # Plot patch reference configuration
            coords_plot = np.append(self._corners_coords_ref,
                                    self._corners_coords_ref[0:1, :], axis=0)
            ax.plot([coords_plot[i, 0] for i in range(len(coords_plot))],
                    [coords_plot[i, 1] for i in range(len(coords_plot))],
                    '-o', color='k', label='Reference configuration')
            # Plot corners boundary conditions
            markers_dim = ('>', '^')
            for i in range(self._n_corners):
                for j in range(self._n_dim):
                    if self._corners_bc[i][j] == 1:
                        coords_plot = self._corners_coords_ref[i, :]
                        ax.plot([coords_plot[0],], [coords_plot[1],], '-',
                                marker=markers_dim[j],
                                markersize=15,
                                markerfacecolor='None',
                                markeredgecolor='#1f77b4',
                                zorder=15)
            # Plot patch deformed configuration (corners)
            coords_plot = np.append(self._corners_coords_def,
            ...
            for i in range(self._n_dim):
                for j, (cid_l, cid_r) in enumerate(
                    ...
                    coords_plot = self._edges_coords_def[str(i)][j]
                    ax.plot([coords_plot[i, 0]
...
class FiniteElementPatchGenerator:
        ...
        if corners_lab_bc is None:
            ...
            for label in corners_lab_bc.keys():
                ...
                if not isinstance(corners_lab_bc[label], tuple):
                    raise RuntimeError('Corner boundary conditions must be '
                                       'prescribed as a tuple[int](n_dim).')
    ...
    def _set_corners_coords_ref(self):
        ...
        if self._n_dim == 2:
            corners_coords[0, :] = [0.0, 0.0]
            corners_coords[1, :] = [dims[0], 0.0]
            corners_coords[2, :] = [dims[0], dims[1]]
            corners_coords[3, :] = [0.0, dims[1]]
        else:
            corners_coords[0, :] = [0.0, 0.0, 0.0]
            corners_coords[1, :] = [dims[0], 0.0, 0.0]
            corners_coords[2, :] = [dims[0], dims[1], 0.0]
            corners_coords[3, :] = [0.0, dims[1], 0.0]
            corners_coords[4, :] = [0.0, 0.0, dims[2]]
            corners_coords[5, :] = [dims[0], 0.0, dims[2]]
            corners_coords[6, :] = [dims[0], dims[1], dims[2]]
            corners_coords[7, :] = [0.0, dims[1], dims[2]]
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    def _polynomial_sampler(order, left_point, right_point, lower_bound=None,
        ...
        """
        # Set number of sampled internal point
