...
* :code:`hookeai/model_architectures`: Directory containing the implementation of all pre-built material model architectures, as well as the associated training and prediction procedures.

* :code:`hookeai/simulators/fetorch/material`: Directory containing the implementation of conventional material models from the FETorch (PyTorch-based Finite Element Method) library.
...
.. note ::
    ...
    In order to enable an efficient material model updating process, HookeAI's global material model updating leverages `PyTorch's vectorizing maps <https://docs.pytorch.org/docs/stable/generated/torch.vmap.html>`_. This requires that the implementation of the conventional material model is compatible with PyTorch vectorization requirements.
